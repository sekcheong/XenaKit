namespace Zena {
	...
	class ThemeSelectViewModel {
		...
		constructor(parent) {
			...
			this.themeProvider(function (e: ctl.EventParam) {
				e.data(that._items);
				e.done();
			});
	...
	export class Application {
		...
		public run() {
			...
			input.onItemClick(function (e: ctl.EventParam) {
				var item: ctl.MenuItem = e.data();
				console.log("click:", item.text(), "[", item.data(), "]");				
			})

			input.onItemsPopulated(function (e: ctl.EventParam) {
			...
			var menu: ctl.Menu = new ctl.Menu("Test", ctl.Menu.MenuType.DEFAULT);
		...
		private setupShortCuts(input, menu) {
			var that = this;
		...
		public callAsync(func, context, ...any) {
			var args = [];
